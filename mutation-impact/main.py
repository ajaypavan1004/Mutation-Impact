# import os
# from mutation_impact.cli import parse_args
# from mutation_impact.io_utils import load_fasta, load_mutations, write_validated
...
# from mutation_impact.aggregation import build_feature_matrix
...
#     write_validated(validated, args.output)
#
#     structure_features = extract_structure_features(args.structure,args.chain,sequence)
#     structure_features.to_csv(os.path.join(args.output, "position_features.csv"),index=False)
#
#     conservation_features = extract_conservation_features(args.fasta,args.homologs,args.output)
#     conservation_features.to_csv(os.path.join(args.output, "conservation_features.csv"),index=False)
#
#     mutation_features = compute_mutation_features(validated)
#     mutation_features.to_csv(os.path.join(args.output, "mutation_features.csv"),index=False)
#
#     feature_matrix = build_feature_matrix(validated,structure_features,conservation_features)
#     feature_matrix = feature_matrix.merge(mutation_features,on=["position", "wt", "mut"],how="left")
#     print(feature_matrix.columns)
#     feature_matrix = compute_stability_score(feature_matrix)
#     feature_matrix.to_csv(os.path.join(args.output, "final_mutation_analysis.csv"),index=False)
#
#
...
def main():
    ...
    ml_feature_matrix = feature_matrix.copy()
    for col in ["stability_score", "stability_class"]:
